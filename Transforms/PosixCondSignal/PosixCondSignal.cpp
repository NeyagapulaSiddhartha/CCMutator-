  ...
  struct PosixCondSignal : public ModulePass {
	...
	if (rmMode) {
		...
		if (calledFuncName == "pthread_cond_signal") {
		    if (curInst->arg_size() >= 1) {
			// The argument of pthread_cond_signal and
			// broadcast is the same
			Value *argZero = curInst->getArgOperand(0);
			//LLVMContext con;
			auto c = M.getOrInsertFunction("pthread_cond_broadcast", 
			    IntegerType::get(M.getContext(), sizeof(int) * 8), // return type
			    argZero->getType(), nullptr); // param 0
			...
			Value *argZero = curInst->getArgOperand(0);
			//LLVMContext con;
			auto c = M.getOrInsertFunction("pthread_cond_signal", 
			    IntegerType::get(M.getContext(), sizeof(int) * 8), // return type
			    argZero->getType(), nullptr); // param 0
			//DEBUG(errs() << "getOrInsertFunction returned " << *c);
			curInst->setCalledFunction(c);
