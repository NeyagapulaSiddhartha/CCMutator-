...
namespace {
class StdMutex : public PassInfoMixin<StdMutex> {
public:
    enum LockUnlockType {
        CallCall,
        CallInvoke,
    ...
    SmallPtrSet<CallInst *, 32> mutateCalls;
    SmallPtrSet<InvokeInst *, 32> mutateInvokes;
    ...
    PreservedAnalyses run(Module &M, ModuleAnalysisManager &AM) {
        ...
        if (rmMode) {
            ...
            for (unsigned i = 0; i < MutatePos.size(); i += 2) {
                unsigned pos1 = MutatePos[i];
                unsigned pos2 = MutatePos[i + 1];
                if (pos1 == 0) {

                LockUnlockPairs::CallCallLockPair *curCallCallPair;
                    curCallCallPair = lockPairs.getCallCallPair(pos2);
                    //auto *curCallCallPair = lockPairs.getCallCallPair(pos2);
                    if (!curCallCallPair) {
  ...
  // TODO: No test case for Invoke-Invoke pair found
        ...
        SmallPtrSet<CallInst *, 32>::iterator Ic = mutateCalls.begin();
	    SmallPtrSet<CallInst *, 32>::iterator Ec = mutateCalls.end();
           for (;Ic != Ec; ++Ic) {
#ifdef MUT_DEBUG
        ...
        SmallPtrSet<InvokeInst *, 32>::iterator Ii = mutateInvokes.begin();
	    SmallPtrSet<InvokeInst *, 32>::iterator Ei = mutateInvokes.end();
            for (;Ii != Ei; ++Ii) {
#ifdef MUT_DEBUG
    ...
    for (unsigned i = 0; i < MutatePos.size(); i += 4) {
        ...
        if (pair2Type == TypeError) {
            errs() << "Warning: type error encountered from position 3, skipping\n";
            continue;
        }
        ...
        if (lockCall1 == lockCall2) {
            errs() << "Warning: lock pair stems from the same lock call, skipping\n";
            continue;
        }
        if (unlockCall1 == unlockCall2) {
            errs() << "Warning: unlock pair stems from the same unlock call, skipping\n";
            continue;
        }
    ...
    errs() << "DEBUG: in shift mode\n";
        ...
        if ((i / 2) < LockDir.size()) {
            int shiftDir = LockDir[i / 2];
            if (shiftDir == 1) {
                errs() << "Warning: a shift of positive 1 is a no-op\n";
            } 
            else {
                if (lockIsCall) {
                    shiftCallInst(cast<CallInst>(lockCall), shiftDir);
                }
                 else {
                    shiftInvokeInst(cast<InvokeInst>(lockCall), shiftDir);
                }
                modified = true;
            }
        }
         else {
            errs() << "Warning: position pair (" << pos1 << ' '
                   << pos2 << ") has no lock shift direction specified\n";
        }
        if ((i / 2) < UnlockDir.size()) {
            int shiftDir = UnlockDir[i / 2];
            if (shiftDir == 1) {
                errs() << "Warning: a shift of positive 1 is a no-op\n";
            }
             else {
                if (unlockIsCall) {
                    shiftCallInst(cast<CallInst>(unlockCall), shiftDir);
                } 
                else {
                    shiftInvokeInst(cast<InvokeInst>(unlockCall), shiftDir);
                }
                modified = true;
            }
        }
    ...
    int swapCalls(CallInst *call1, CallInst *call2) {
        ...
        //         iter1, call2Copy);
         ReplaceInstWithInst(call1, call2Copy);
        ...
        // llvm::ReplaceInstWithInst(call2->getParent()->getInstList(),
        //         iter2, call1Copy);
        llvm::ReplaceInstWithInst(call2, call1Copy);
    ...
    int swapInvokes(InvokeInst *invoke1, InvokeInst *invoke2) {
        ...
        //         iter1, invoke1Func2);
       ReplaceInstWithInst(invoke1, invoke1Func2);
        BasicBlock::iterator iter2(invoke2);
        // ReplaceInstWithInst(invoke2->getParent()->getInstList(),
        //         iter2, invoke2Func1);
                ReplaceInstWithInst(invoke2,invoke2Func1);
    ...
    void insertInstructionRelative(Instruction *base, Instruction *insertMe, unsigned distance) {
	    // Obtain the iterator to the 'base' instruction
    Instruction *target = base;

    // Advance 'distance' instructions forward
    for (unsigned i = 0; i < distance; i++) {
        if (Instruction *next = target->getNextNode()) {
            target = next;
        } else {
            errs() << "Warning: insertInstructionRelative() reached end of basic block "
                      "before distance was reached\n";
            break;
        }
    }

    // Insert the instruction before the target position
    insertMe->insertBefore(target);
    }
...
llvm::PassPluginLibraryInfo getStdMutexPluginInfo() {
    return {LLVM_PLUGIN_API_VERSION, "Mutex", LLVM_VERSION_STRING, [](llvm::PassBuilder &PB) {
